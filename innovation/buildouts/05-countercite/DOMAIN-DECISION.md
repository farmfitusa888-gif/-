# Countercite: the domain decision
...
**Buy a Countercite `.com` variant, use it for both the website and the cold
...
|---|---|---|---|---|---|
| A Countercite `.com` | Cloudflare (at cost) | ~$10.44 | ~$11.15 | ~$11.15 | **~$32.75** |
| A Countercite `.com` | Porkbun (flat renewal) | $11.80 | ~$12.50 | ~$12.50 | **~$36.80** |
| `countercite.app` | Dynadot | $10.00 | $14.50 | $14.50 | **$39.00** |
| `countercite.app` | Porkbun | $9.81 | $14.93 | $14.93 | **$39.67** |
| `countercite.app` | Namecheap | $10.98 | [NOT ESTABLISHED] | [NOT ESTABLISHED] | [NOT ESTABLISHED] |
| `countercite.tools` | Dynadot | $9.85 | $30.18 | $30.18 | **$70.21** |
| `countercite.tools` | Namecheap | $8.48 | [NOT ESTABLISHED] | [NOT ESTABLISHED] | [NOT ESTABLISHED] |
...
second. `countercite.tools` reads like a hardware supplier and is the weakest
of the three on this axis by a distance. `countercite.app` reads as software
...
up an existing product at **`countercite-app.com`** described as immutable
...
your buyers under a near-identical name. `countercite.app` and
`countercite-app.com` are the same string to a human being. Also live:
`countercitemedia.com`, `countercite.org` (weight management),
`countercite.store`. The name is crowded. Verify the `countercite-app.com`
...
1. **`countercite.com` is probably gone, and the good variants may be too.**
...
2. **A prefixed `.com` is a visible compromise.** `getcountercite.com` tells a
   sharp reader that someone else owns `countercite.com`. Prefixes like `get`
...
3. **The name collision is the real problem and `.com` does not fix it.**
   If `countercite-app.com` is genuinely selling to insurance adjusters, no
   domain choice rescues the name. Consider whether Countercite survives at
...
**Monday.** Verify the `countercite-app.com` collision. Open it, see who they
...
|---|---|---|
| 1 | `countercitecase.com` | Descriptive, no prefix, says what it does |
| 2 | `counterciteclaims.com` | Reads native to the buyer |
| 3 | `usecountercite.com` | Standard SaaS convention |
| 4 | `getcountercite.com` | Same, slightly more marketing-flavoured |
| 5 | `trycountercite.com` | Keep as the second sending domain |
...
# Decision update, 2026-08-31: the name changed, and the reasoning survives it

The brand is now Countercite. What this document argued about `.com` versus
`.app` did not depend on the name, so it still stands, and it now has more
weight behind it rather than less. The Florida list came back at **3,708 licensed
public adjusters, every one with a published email address**. That is a bigger
one-shot asset than the 1,203 firms this document was written against, and it
raises the cost of landing in quarantine on the first send.

So the recommendation does not flip to `.app` just because `countercite.app`
happens to be free. Deliverability still wins over tidiness.

## The decision rule, in order

Check these three at a registrar. It takes under a minute and settles it.

1. **`countercite.co`** — if free, buy it and use it for everything. `.co` is
   nineteen years old, universally recognised, and carries none of the filtering
   risk this document catalogued for newer gTLDs. One domain, one brand, no split.
2. **`getcountercite.com`** — if `.co` is gone, this is next. A `get` prefix is
   common enough among software companies that it reads as normal rather than as
   a workaround.
3. **`countercite.app`** — confirmed available. Take it if the first two are gone,
   and then treat domain warmup as mandatory rather than optional before the
   Florida send: three weeks of low-volume sending, SPF, DKIM and DMARC in place
   from day one, and the first cold batch capped at fifty a day.

**Buy `countercite.app` regardless of which one wins**, for about $12. If it ends
up as the secondary it redirects to the primary and costs nothing to hold. If a
competitor picks it up later, the confusion is exactly what the rename was meant
to avoid.

## What not to do

Do not run the product on one domain and the cold outreach from another. The
mismatch between the address in the From line and the address on the website is
noticed by recipients and by filters, and it undoes the deliverability advantage
that motivated buying the `.com` in the first place. Pick one and use it
everywhere.

## Why the rename happened

`counterweight-app.com` is a live company selling C2PA provenance certification,
and its served-market list names insurance adjusters. Same buyer, different
product, one hyphen apart. Full reasoning and the alternatives considered are in
`../00-NAMES.md`.
